...
Protected Module USTAR
	...
	#tag Method, Flags = &h0
		Function IsTarred(Extends TargetFile As FolderItem) As Boolean
		  ...
		  Try
		    #If USE_ZLIB Then
		      If TargetFile.IsGZipped Then bs = zlib.CompressedStream.Open(TargetFile, zlib.GZIP_ENCODING)
		    #endif
		    #If USE_BZIP Then
		  ...
		  Finally
		    If bs <> Nil Then
		      ...
		      #If USE_ZLIB Then
		        If bs IsA zlib.CompressedStream Then zlib.CompressedStream(bs).Close
		      #endif
		      #If USE_BZIP Then
	...
	#tag Method, Flags = &h0
		Function IsTarred(Extends Target As MemoryBlock) As Boolean
		  ...
		  Try
		    #If USE_ZLIB Then
		      If Target.IsGZipped Then bs = New zlib.CompressedStream(Target)
		    #endif
		    #If USE_BZIP Then
	...
	#tag Method, Flags = &h1
		Protected Function ReadTar(TarFile As FolderItem, ExtractTo As FolderItem, Overwrite As Boolean = False) As FolderItem()
		  ...
		  Dim ts As Readable
		  #If USE_ZLIB Then
		    If TarFile.IsGZipped Then ts = zlib.CompressedStream.Open(TarFile, zlib.GZIP_ENCODING)
		  #endif
		  #If USE_BZIP Then
		  ...
		  #If USE_ZLIB Then
		    If ts IsA zlib.CompressedStream Then zlib.CompressedStream(ts).Close
		  #endif
		  #If USE_BZIP Then
	...
	#tag Method, Flags = &h1
		Protected Function WriteTar(ToArchive() As FolderItem, OutputFile As FolderItem, Optional RelativeRoot As FolderItem, Overwrite As Boolean = False, CompressionLevel As Integer = 0) As Boolean
		  ...
		  #If USE_ZLIB Then
		    If t IsA zlib.CompressedStream Then zlib.CompressedStream(t).Close
		  #endif
		  #If USE_BZIP Then
